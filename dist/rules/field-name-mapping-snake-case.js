...
import { getMappedName, looksLikeAssociationFieldType, } from '#src/common/prisma.js';
...
function findMapAttribute(attributes) {
    const filtered = attributes.filter((a) => a.name === 'map');
    if (filtered.length === 0) {
        return;
    }
    if (filtered.length > 1) {
        throw Error(`Unexpected multiple map attributes! ${JSON.stringify(filtered)}`);
    }
    return filtered[0];
}
