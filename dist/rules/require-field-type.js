...
export default {
    ...
    create: (config, context) => {
        ...
        return {
            Field: (model, field) => {
                const matches = requireWithRegExp.filter((r) => r.ifNameRegExp.test(field.name));
                if (matches.length === 0) {
                    return;
                }
                const areMatchesConflicting = new Set(matches.map(({ type }) => type)).size > 1 ||
                    new Set(matches.map(({ nativeType }) => nativeType).filter(Boolean))
                        .size > 1;
                if (areMatchesConflicting) {
                    const message = `Field has conflicting type require: ${JSON.stringify(matches.map(({ ifName, type, nativeType }) => ({
                        ifName,
                        type,
                        nativeType,
                    })))}.`;
                ...
                if (actualType !== expectedType) {
                    messages.push(`Field type "${actualType}" does not match expected type "${expectedType}".`);
                }
                const expectedNativeType = matches[0].nativeType;
                const actualNativeType = field.attributes?.find((attr) => attr.group === 'db')?.name;
                if (expectedNativeType && !actualNativeType) {
                    messages.push(`Field native type is not specified, but expected native type "${expectedNativeType}".`);
                }
                else if (expectedNativeType &&
                    actualNativeType !== expectedNativeType) {
                    messages.push(`Field native type "${actualNativeType}" does not match expected native type "${expectedNativeType}".`);
                }
