...
function hasViolation(field) {
    ...
    if (firstArgValue.type !== 'array') {
        return true;
    }
    if (firstArgValue.args != null) {
        return true;
    }
    return false;
}
function findDefaultAttribute(attributes) {
    const filtered = attributes.filter((a) => a.name === 'default');
    if (filtered.length === 0) {
        return;
    }
    if (filtered.length > 1) {
        throw Error(`Unexpected multiple default attributes! ${JSON.stringify(filtered)}`);
    }
    return filtered[0];
}
