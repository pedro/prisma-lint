...
export default {
    ...
    create: (config, context) => {
        ...
        const requireNames = require.filter((f) => typeof f === 'string');
        const conditions = require.filter((f) => typeof f === 'object');
        const simpleIfSiblingConditions = conditions.filter((f) => !isRegexOrRegexStr(f.ifSibling));
        const regexIfSiblingConditions = conditions
            .filter((f) => isRegexOrRegexStr(f.ifSibling))
            .map((f) => {
            const { ifSibling } = f;
            const ifSiblingRegExp = typeof ifSibling === 'string'
        ...
        return {
            Model: (model) => {
                ...
                for (const condition of simpleIfSiblingConditions) {
                    ...
                    if (fieldNameSet.has(condition.ifSibling) &&
                ...
                for (const condition of regexIfSiblingConditions) {
                    ...
                    if (fields.some((f) => condition.ifSiblingRegExp.test(f.name)) &&
                        !fieldNameSet.has(condition.name)) {
                        missingFields.push(condition.name);
                    }
                }
