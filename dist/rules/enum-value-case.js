...
import { matchesAllowList, trimPrefix, } from '#src/common/rule-config-helpers.js';
...
export default {
    ...
    create: (config, context) => {
        const { allowList, trimPrefix: trimPrefixConfig, compoundWords, case: caseConfig, style, } = config;
        return {
            Enum: (enumObj) => {
                enumObj.enumerators
                    .filter((enumerator) => enumerator.type === 'enumerator')
                    .forEach((enumValue) => {
                    if (getRuleIgnoreParams(enumObj, RULE_NAME).includes(enumValue.name)) {
                        return;
                    }
