    ...
    create: (config, context) => {
        ...
        return {
            Field: (model, field) => {
                ...
                const nativeTypeAttribute = field.attributes?.find((attr) => attr.group === 'db');
                if (!nativeTypeAttribute ||
                    (nativeTypeAttribute.name === 'Text' && !allowNativeTextType)) {
                    const message = 'String fields must have a native type attribute to ensure maximum length, e.g. `@db.VarChar(x)`.';
