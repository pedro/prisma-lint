...
import { assertValueIsArray, isFunc, isKeyValue, isValue, listAttributes, listFields, } from '#src/common/prisma.js';
...
export default {
    ...
    create: (config, context) => {
        ...
        const indexSetByModelName = new Map();
        const relationSetByModelName = new Map();
        return {
            Field: (model, field) => {
...
function extractRelationSet(model) {
...
function extractIndexSet(model) {
...
function isIdField(field) {
    return Boolean(field.attributes?.find((attribute) => attribute.name === 'id'));
}
function isUniqueField(field) {
    return Boolean(field.attributes?.find((attribute) => attribute.name === 'unique'));
}
function extractPrimaryFieldNameFromRelationListAttribute(attribute) {
    let value;
    ...
    if (value === undefined) {
        throw new Error(`Failed to parse attribute, could not find fields in ${JSON.stringify(attribute)}`);
    }
...
function extractRelationFieldNames(field) {
    const relationAttribute = field.attributes?.find((attribute) => attribute.name === 'relation');
    ...
    const fieldsArgValue = fieldsArg.value.value;
    return assertValueIsArray(fieldsArgValue);
}
