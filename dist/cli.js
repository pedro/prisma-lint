...
program
    ...
    .option('-o, --output-format <format>', 'Output format. Options: simple, contextual, json, filepath, none.', 'simple')
    .option('--no-color', 'Disable color output.')
    .option('--quiet', 'Suppress all output except for errors.')
    .argument('[paths...]', 'One or more schema files, directories, or globs to lint.', DEFAULT_PRISMA_FILE_PATH);
program.parse();
const explorer = cosmiconfig('prismalint');
const options = program.opts();
const { args } = program;
const getSchemaFromPackageJson = async (cwd) => {
 ...
 */
const getSchemaFromConfig = async (cwd) => {
...
const getPathsFromArgsOrConfig = async (args) => {
    if (args.length > 0) {
        return args;
    }
...
const resolvePrismaFileNames = (paths) => {
    const resolvedFiles = [];
...
const outputParseIssues = (filepath, parseIssues) => {
...
const run = async () => {
    ...
    if (rootConfig == null) {
        // eslint-disable-next-line no-console
        console.error('Unable to find configuration file for prisma-lint. Please create a ".prismalintrc.json" file.');
        process.exit(1);
    }
    const { rules, parseIssues, deprecationWarnings } = parseRules(ruleDefinitions, rootConfig.config);
    ...
    const hasViolations = fileViolationList.some(({ violations }) => violations.length > 0);
    if (hasViolations) {
        process.exit(1);
    }
};
